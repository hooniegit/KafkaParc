...
public class StateService {
    ...
    private final String bufferPath = "/Users/a1234/Buffer/StatusTwo/";
    ...
    public void check(List<TagData<String>> dataList) {
        ...
        for (TagData<String> data : dataList) {
            String origin = this.referenceMap.getIdMap().get(data.getId());
            if (origin.equals("NULL")) continue;
            if (!origin.equals(data.getValue())) {
                if (data.getValue() == null) {
                    entryList.add(new TagData<>(data.getId(), "NULL", data.getTimestamp()));
                    System.out.println("[BF]" + origin + " -> [AF]NULL");
                    continue;
                };
                System.out.println("[BF]" + origin + " -> [AF]" + data.getValue());
                entryList.add(data);
            }
        }
